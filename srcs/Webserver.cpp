...
void Webserver::init(const std::vector<ServerConfig> &configs)
{
    ...
    for (size_t i = 0; i < configs.size(); ++i)
    {
        ...
        {
            if (listening_ports[j] == port) { port_exists = true; break; }
        }
...
void Webserver::initSocket(int port)
{
    ...
    if (fcntl(server_fd, F_SETFL, O_NONBLOCK) < 0) { perror("fcntl"); exit(EXIT_FAILURE); }
    ...
    if (listen(server_fd, 10) < 0) { perror("listen"); close(server_fd); exit(EXIT_FAILURE); }
        ...
        for (size_t i = 0; i < _fds.size(); ++i)
        {
            ...
            if (_fds[i].revents & POLLIN)
            {
                ...
                for (size_t j = 0; j < _server_fds.size(); ++j) {
                    if (fd == _server_fds[j]) { is_server = true; break; }
                }
...
void Webserver::acceptConnection(int server_fd)
{
    ...
    if (fcntl(client_fd, F_SETFL, O_NONBLOCK) < 0) { perror("fcntl client"); close(client_fd); return; }
